...
// Army.prototype.getHorses = function() {
//     return this.horses;
// }

// Army.prototype.getElephants = function() {
//     return this.elephants;
// }

// Army.prototype.getArmouredTanks = function() {
//     return this.armouredTanks;
// }

// Army.prototype.getSlingGuns = function() {
//     return this.slingGuns;
// }
...
let defenseArmy = function(attackArmyObj, defenseArmyObj) {
    ...
    if ( attackArmyArr.length === defenseArmyArr.length ) {
        ...
        for( let i = 0; i < attackArmyArr.length; i++ ) {
            ...
            if ( attackUnits < defenseUnits ) {                
                ...
                spareUnits[i] = 0;
                backUpUnits[i] = attackUnits - defenseUnits;
            }
        }
        ...
        for (var i = 0; i < attackArmyArr.length; i++) {
            switch (i) {
                case 0: 
                    if (backUpUnits[i] > 0 && spareUnits[i+1] > 0) {
                        newDefenseArmyArr[i+1] = ((backUpUnits[i]/2) < spareUnits[i+1]) ? Math.ceil(newDefenseArmyArr[i+1] + (backUpUnits[i]/2)) : defenseArmyArr[i+1];
                    }
                    break;
                case 1:
                case 2: 
                    if (backUpUnits[i] > 0 && spareUnits[i-1] > 0) {
                        newDefenseArmyArr[i-1] = ((backUpUnits[i]*2) < spareUnits[i-1]) ? (newDefenseArmyArr[i-1] + (backUpUnits[i]*2)) : defenseArmyArr[i-1];
                    } else if (backUpUnits[i] > 0 && spareUnits[i+1] > 0) {
                        newDefenseArmyArr[i+1] = ((backUpUnits[i]/2) < spareUnits[i+1]) ? Math.ceil(newDefenseArmyArr[i+1] + (backUpUnits[i]/2)) : defenseArmyArr[i+1];
                    } 
                    break;
                case 3: 
